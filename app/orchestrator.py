...
async def cleanup_job():
    """Удаляет устаревшие посты из очереди (старше 6 часов)."""
    await safe_run("cleanup", lambda: publisher.cleanup_old_queued())
...
def setup_scheduler() -> AsyncIOScheduler:
    ...
    # Публикация — раз в 30 минут
    sched.add_job(publish_job, IntervalTrigger(minutes=30), id="publisher", max_instances=1, coalesce=True)
    # Очистка устаревших постов — раз в час
    sched.add_job(cleanup_job, IntervalTrigger(hours=1), id="cleanup", max_instances=1, coalesce=True)
